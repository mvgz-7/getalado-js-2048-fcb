...
// If one of these variables value became true, the player already won
...
function setGame(){
	...
	for(let r=0; r < rows; r++){
		for(let c=0; c < columns; c++){
			...
			// Create a tile through creating div elements
...
// If nonzero, nilalagay siya sa isang array
// Ex. [0, 2, 2, 2] -> [2, 2, 2]
...
function handleSlide(e){
	...
	if(["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.code)){
		...
		document.getElementById("score").innerText = score; // updating the score sa id="score" in html
	}
...
function hasEmptyTile(){
	for (let r = 0; r<rows; r++){
		for(let c = 0; c<columns; c++){
			...
			if(board[r][c] == 0){
				return true; //merong zero
			}
		}
	}

	return false; // walang zero
}
    ...
    function canMoveLeft() {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 1; c < columns; c++) {
                console.log(`${r} - ${c}`); // r + "-" + c // $ -> This is an update from JS ES6 when executing Concatenation
